...
As an example, I display the percent of students clearly behind grade for White, Black, and Black/White students in @fig-example. The point estimate and confidence interval for the Black/White group are shown in red, while the two monoracial groups are shown via grey dots and confidence bands. For all three cases, I use 83.4% confidence intervals, because overlap on such intervals indicates that the observed difference between the two measures in the sample is not statistically distinguishable from zero in the population at $p<.05$ [@knol_mis_2011].[^2] In this case, the lack of overlap between any of the confidence bands indicates that the Black/White probability of being clearly behind expected grade is statistically distinguishable from both monoracial probabilities.

[^2]: The correspondence to $p<.05$ is only approximate due to differences in the standard errors between groups. Calculating more precise intervals requires that those intervals vary across each pairwise comparison which does not work for the visualizations here. The approximation will be very close in all cases used here.
...
[^3]: The halfway point is given by taking the mean between the two monoracial group. Its standard error is given by $\sqrt{(s_1^2+s_2^2)/2}$ where $s_1$ and $s_2$ are the standard errors of the estimates for the two monoracial groups.
...
screenreg(map(results_monoracial[-1], convert_marg),
       digits=3,
