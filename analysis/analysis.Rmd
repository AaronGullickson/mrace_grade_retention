...
```{r}
#try a model
model1 <- glm(below_exp_grade~race+as.factor(year)+foreign_born, data=acs, 
              family=binomial)
...
coef_table <- bind_rows(tidy(model1) %>%
                        ...
                        tidy(model4) %>%
                          filter(str_detect(term, "race") & 
                                   ...
                                   !str_detect(term, "Multiracial")) %>%
                          mutate(model="model4"))
ggplot(coef_table, aes(x=reorder(term, estimate, mean), y=estimate, color=model, group=model))+
