...
model1 <- update(model0, 
                 .~.+as.factor(year)+current_grade+as.factor(state)+metro+sex)
...
Ok, let me try to summarize these results, by group.
...
Why does the big change occur in the parental resources model. The descriptive stat table shows that the family resources of Black/White biracials are closer to those of black students than white students. So, Black/White biracials are at a disadvantage relative to whites, which is controlled for in the last model.
...
Things change quite a bit in the parent resources model. The gap between blacks and Latinos grows and the Black/Latino group now looks almost identical to the Latino group. So, some of their lower rate was due to better family resources, particularly relative to Latinos, as can be seen on the table of descriptive stats.
...
```{r monoracial-results, warning=FALSE}
