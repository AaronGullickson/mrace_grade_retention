...
```{r}
#try a model
model0 <- glm(below_exp_grade~as.factor(year)+current_grade, 
...
model4a <- update(model4, data=subset(acs, as.numeric(current_grade)<=7))
model4b <- update(model4, data=subset(acs, as.numeric(current_grade)>7 &
                                        as.numeric(current_grade)<10))
...
```{r}
#make a plot showing difference from middle expectation by model
coef_table <- bind_rows(tidy(model1) %>%
                          filter(str_detect(term, "race") & 
                                   str_detect(term, "/") & 
                                   !str_detect(term, "Multiracial")) %>%
                          mutate(model="model1"),
                        tidy(model2) %>%
                          filter(str_detect(term, "race") & 
                                   str_detect(term, "/") & 
                        ...
                        tidy(model4) %>%
                          filter(str_detect(term, "race") & 
                                   ...
                                   !str_detect(term, "Multiracial")) %>%
                          mutate(model="model4"))
ggplot(coef_table, aes(x=reorder(term, estimate, min), y=estimate, color=model, group=model))+
