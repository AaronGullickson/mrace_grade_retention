...
```{r check-retention-code, caption="The proportion of students measured at below grade level by age and current grade. This graph is used to ensure that the dependent variable is calculated correctly."}
acs %>%
...
```{r retention-by-grade, caption="Percent of students clearly behind grade level by current grade. Blue lines fit a smoothed curve to the data."}
acs %>%
...
```{r retention-by-year, caption="Trends over time in the percent of students clearly behind grade level. Blue lines fit a smoothed curve to the data."}
acs %>%
  group_by(year) %>%
  summarize(below_grade=mean(below_exp_grade)) %>%
...
```{r retention-by-year-grade, caption="Trends over time and grade in the percent of students clearly behind grade level. Lines are calculated by smoothing."}
acs %>%
...
knitreg(map(results_monoracial, convert_marg),
        caption="Differences in the probability of being behind grade by race based on various statistical models. All estimates are average marginal effects.",
...
```{r example-plot, caption="Probability of being behind grade for biracial black/white respondents in comparison to their monoracial comparison groups. 83.4% confidence bands shown around each estimate. Non-overlap in confidence bands roughly indicates statistically significant difference at the 5% level"}
g <- coef_table_mod0 %>%
...
```{r cult-resource-model, warning=FALSE, caption="Average cultural resources for biracial respondents in comparison to their monoracial comparison groups. 83.4% confidence bands shown around each estimate. Non-overlap in confidence bands roughly indicates statistically significant difference at the 5% level. Cultural resources are determined by a summated score of standardized native-born status, parents' native-born status, English proficieny, and parents' English proficiency. Parents' native-born status is reverse coded based on model results which show it increases grade retention risk."}
...
```{r fam-resource-model, warning=FALSE, caption="Average family resources for biracial respondents in comparison to their monoracial comparison groups. 83.4% confidence bands shown around each estimate. Non-overlap in confidence bands roughly indicates statistically significant difference at the 5% level. Family resources are determined by a summated score of standardized family income, parental education, and home ownership."}
#If I use the glm framework then I should be able to feed this back into
#the marg command
